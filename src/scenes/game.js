...
import { spawnOnMapEdge } from "../utils/map";
...
class GameScene extends Phaser.Scene {
  ...
  handleCursor() {
    ...
    if (
      this.input.manager.activePointer.justUp &&
  ...
  setUpCursor() {
    ...
    this.input.setDefaultCursor(
      "url(assets/spritesheets/cursor_pointer.png), pointer"
    );
  }
  ...
  createCryptominerGroup(numberOfMiners) {
    ...
    for (let i = 0; i < numberOfMiners; i++) {
      const plantObj = new CryptoMiner(this, 0, 0, {
        key: "cryptominer",
        active: false
      });
